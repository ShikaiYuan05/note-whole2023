...
> `前端工程化`是使用`软件工程的方法`来`单独`解决前端的开发流程中`模块化、组件化、规范化、自动化`的问题,其主要目的为了提高效率和降低成本。 
...
+ 前端工程师的知识门槛变高了
+ 前端工程化的生态还在没有完全成熟,技术迭代速度比较快,程序员需要及时适应
+ ... ...

### 1.3.前端工程化实现技术栈

> 前端工程化实现的技术栈有很多,我们采用ES6+nodejs+npm+Vite+VUE3+Element-plus组合来实现
...
> 普通函数: 正常调用的函数,一般函数执行完毕后才会继续执行下一行代码(阻塞线程)
...
#### 2.6.7 async和await结合使用(读取文件)

![1684478815640](images/1684478815640.png)

```html
<script>
    //声明一个根据url读取对应文件的方法
    function loatTextFile(url){
        let res =fetch(url).then(response =>{
            if(response.ok){
                return  response.text()
            } 
            throw new Error(`file not found in ${response.url} with status ${response.status}`)
        })
        return res
    }
    // 调用方法,获取多个文件中的内容
    async function main(){
        ...
        let res3
        try {
            res = await loatTextFile('./resources/a.txt')
        }catch(e){
            console.log(e)
        }
        try {
            res2 = await loatTextFile('./resources/b.properties')
        }catch(e){
            console.log(e)
        }
        try {
            res3 = await loatTextFile('./resources/c.js')
        }catch(e){
            console.log(e)
        }
        return res+'\r\n'+res2+'\r\n'+res3
...
> 目前，前端模块化有多种规范和实现，包括 CommonJS、AMD 和 ES6 模块化。其中 CommonJS 最初是为了解决 Node.js 端的模块化问题而产生，通过 require() 来引入模块，通过 module.exports 导出模块。AMD 则是为了解决浏览器端的异步加载问题，通过 define() 来定义模块，通过 require() 来加载依赖。`ES6 模块化是 JavaScript 语言的模块标准，使用 import 和 export 关键字来实现模块的导入和导出。现在，大部分浏览器都已经原生支持 ES6 模块化，因此它成为了最为广泛使用的前端模块化标准. `
...
![1684480894743](images/1684480894743.png)
...
+ 在 Windows 系统上，npm 的全局依赖默认安装在 `<用户目录>\AppData\Roaming\npm` 目录下。例如，如果当前用户的用户名为 `test`，那么全局依赖将会安装在 `C:\Users\test\AppData\Roaming\npm` 目录下
...
+ npm init
    + 进入一个vscode创建好的项目总  执行 npm init 命令后，npm 会引导您在命令行界面上回答一些问题,例如项目名称、版本号、作者、许可证等信息，并最终生成一个package.json 文件。packagejson信息会包含项目基本信息！类似maven的pom.xml
+ npm init -y
...
+ Vue 的单文件组件 (即 *.vue 文件，英文 Single-File Component，简称 SFC) 是一种特殊的文件格式，使我们能够将一个 Vue 组件的模板、逻辑与样式封装在单个文件中。 .vue文件可以被vue加载进行显示!
...
<script type="module" setup>
   //存储vue页面逻辑js代码
   import {ref} from 'vue'
   //setup语法糖 直接在script添加setup属性即可,自动将内部声明输出导出到vue
   let counter = ref(1);
   function increase(){
     counter.value++;
   }
...
<template>
  <div>
    ...
    <!-- 原生js方式阻止组件默认行为 (推荐) -->
    <a href="http://www.atguigu.com" target="_blank" @click.prevent="incrCount($event)">prevent</a> <br>
  </div>
</template>
...
<img src="images/image_elceCM4Wbp.png" style="zoom: 40%;" />
...
+ Vue 的单文件组件是网页开发中 HTML、CSS 和 JavaScript 三种语言经典组合的自然延伸。<template>、<script> 和 <style> 三个块在同一个文件中封装、组合了组件的视图、逻辑和样式。
    + 每个 *.vue  文件中,当我们想要使用内置指令而不在 DOM 中渲染元素时，<template>标签可以作为占位符使用。 
    +  每个 *.vue 文件最多可以包含一个 <script> 块。(使用 <script setup> 的情况除外),<script setup> 中的顶层绑定都将自动暴露给模板
    + 每个 *.vue 文件可以包含多个 <style> 标签。一个 <style> 标签可以使用 scoped 或 module attribute 使用了不同封装模式的多个 <style> 标签可以被混合入同一个组件。

> 2  **为什么要使用 SFC？**

+ 使用 SFC 必须使用构建工具（vite/webpack等），但作为回报带来了以下优点：
    + 使用熟悉的 HTML、CSS 和 JavaScript 语法编写模块化的组件
    + [让本来就强相关的关注点自然内聚](https://cn.vuejs.org/guide/scaling-up/sfc.html#what-about-separation-of-concerns "让本来就强相关的关注点自然内聚")
    + 预编译模板，避免运行时的编译开销
    + [组件作用域的 CSS](https://cn.vuejs.org/api/sfc-css-features.html "组件作用域的 CSS")
    + [在使用组合式 API 时语法更简单](https://cn.vuejs.org/api/sfc-script-setup.html "在使用组合式 API 时语法更简单")
    + 通过交叉分析模板和逻辑代码能进行更多编译时优化
    + [更好的 IDE 支持](https://cn.vuejs.org/guide/scaling-up/tooling.html#ide-support "更好的 IDE 支持")，提供自动补全和对模板中表达式的类型检查
    + 开箱即用的模块热更新 (HMR) 支持
        SFC 是 Vue 框架提供的一个功能，并且在下列场景中都是官方推荐的项目组织方式：
    + 单页面应用 (SPA)
    + 静态站点生成 (SSG)
    + 任何值得引入构建步骤以获得更好的开发体验 (DX) 的项目.当然，在一些轻量级场景下使用 SFC 会显得有些杀鸡用牛刀。因此 Vue 同样也可以在无构建步骤的情况下以纯 JavaScript 方式使用，直接在页面中引入Vue进行传统方式编写！



> 3  **SFC 是如何工作的？**

+ Vue SFC 是一个框架指定的文件格式，因此必须交由 [@vue/compiler-sfc](https://github.com/vuejs/core/tree/main/packages/compiler-sfc "@vue/compiler-sfc") 编译为标准的 JavaScript 和 CSS，一个编译后的 SFC 是一个标准的 JavaScript(ES) 模块，这也意味着在构建配置正确的前提下，你可以像导入其他 ES 模块一样导入 SFC：
...
> 4  一个 Vue 组件在使用前需要先被“注册”，这样 Vue 才能在渲染模板时找到其对应的实现。组件注册有两种方式：全局注册和局部注册，但是我们了解即可，vue3 setup免注册！
...
<script setup>
    ...
    import ComponentA from './components/CompinentA.vue'
    import ComponentB from './components/CompinentB.vue'
    import ComponentC from './components/CompinentC.vue'
    const app = createApp({})
    //全局注册，可以链式调用
    app
...
```html
<script setup>
    //在使用 `<script setup>` 的单文件组件中，导入的组件可以直接在模板中使用，无需注册：(setup默认自动局部注册)
...
-   Vue Router 是 [Vue.js](https://vuejs.org/ "Vue.js") 的官方路由
-   嵌套路由映射
-   动态路由选择
-   模块化、基于组件的路由配置
-   路由参数、查询、通配符
...
+ 路由最终显示区域
    + router-view标签可以有一个或者多个，多个需要通过name名称指定！
    + 最终在打开组件的时候，可以指定使用哪个router-view
    + 通常来说，我们都写成一个！默认使用它打开组件！

```  html
<template>
  <div>
      <!-- 点击出发路由的页面标签！ to指定出发路径 -->
...
const routes = [
  ...
  },
  {
    ...
    components: {
      default: About,  //默认打开
      sidebar: Sidebar //sidebar的router-view打开
    }
  }
]
```
...
+ `<router-link to="/list">list页</router-link>  `这种路由,to中的内容是死的,点击后只能切换/list对象组件(声明式路由)
...
<template>
    <div>
      ...
      <hr/>
        ...
        <input type="text" v-model="routePath" @change="showView()"> <br>
      <hr/>
      <!-- 路由连接对应视图的展示位置 -->
      <hr>
      默认展示位置:<router-view></router-view>
...
<template>
    <div>
      ...
      <br>
      <router-link to="/showDetail/2/Python">showDetail路径传参显示Python</router-link> 
      <button @click="showDetail(2,'Python')">showDetail动态路由路径传参显示Python</button>
      <br>
      <router-link to="/showDetail/3/JavaScript">showDetail路径传参显示JavaScript</router-link> 
      <button @click="showDetail(3,'JavaScript')">showDetail动态路由路径传参显示JavaScript</button>
      <br>
      <hr/>
      <router-link v-bind:to="{path:'/showDetail2',query:{id:1,language:'Java'}}">showDetail2键值对传参显示JAVA</router-link> 
      <button @click="showDetail2(1,'JAVA')">showDetail动态路由键值对传参显示JAVA</button>
      <br>
      <router-link v-bind:to="{path:'/showDetail2',query:{id:2,language:'Python'}}">showDetail2键值对传参显示Python</router-link> 
      <button @click="showDetail2(2,'Python')">showDetail动态路由键值对传参显示Python</button>
      <br>
      <router-link :to="{path:'/showDetail2',query:{id:3,language:'JavaScript'}}">showDetail2键值对传参显示JavaScript</router-link> 
      <button @click="showDetail2(3,'JavaScript')">showDetail动态路由键值对传参显示JavaScript</button>
      <br>
      <hr>
      <hr>
...
> 什么是状态管理？ 状态即是数据和数据操作方法的统称，类似Java的实体类概念！理论上来说，每一个 Vue 组件实例都已经在“管理”它自己的响应式状态了。我们以一个简单的计数器组件为例：
...
> 然而，当我们有`多个组件共享一个共同的状态`时，就没有这么简单了：多个视图可能都依赖于同一份状态。来自不同视图的交互也可能需要更改同一份状态。虽然我们的手动状态管理解决方案（props,组件间通信,模块化）在简单的场景中已经足够了，但是在大规模的生产应用中还有很多其他事项需要考虑：
...
<template>
    <div>
        ...
        请增加爱好:<input type="text" @change="addHobby" v-model="hobby"> <br>  
...
<script setup type="module">
  ...
  let getLoveMessage =()=>{
    ...
    .then(
      /* response代表真个响应,其中的data属性才是正常响应回来的数据 */
      response =>{
        console.log(response.data)
        // 将响应的数据放入一个reactive响应式对象上
        // 不可以直接给JsonData=response.data这样整体赋值,会让JsonData编程非响应式
        //jsonData.code=response.data.code
        //jsonData.content=response.data.content
        //Object.assign将响应的数据赋予给数据,会保留响应式状态,但是要注意属性名要一致(类似java的反射)
...
instance.interceptors.request.use(
    ...
    config=>{
        ...
        //config.headers.z = 'application/json, text/plain, text/html,*/*'
        //返回config
        return config
    },
    // 设置请求错误处理函数
...
+ 我们本次使用pinia存储json数据和对应的操作方法！创建pinia存储文件 store.js
...
-   点击删除按钮，出发删除方法，传入要删除的id List.vue
...
+ **增加两个数据**： loaded是为了控制防止多次本地json加载  ,maxId记录最大数据的ID值,用于在新增时维护元素id,一直累加即可

+ **修改getList方法** 添加是不是否在加载，是加载json文件，不是不加载！并且首次加载赋ID值！
